...
public:
    ...
    void paint(juce::Graphics& g) override
    {
        ...
        gradient.addColour(0.0, juce::Colours::deeppink);
        gradient.addColour(1.0, juce::Colours::aqua);
        ...
        for (int i = 0; i < numVertices; ++i)
        {
            for (int j = 0; j < numVertices; ++j)
            {
                ...
                if (weight > 0)
                {
                    ...
                    if (input.size() > 1)
                    {
                        Text += juce::String(i) + juce::String(j);
                        Text1 += juce::String(j) + juce::String(i);
                        if (numberText.contains(Text) || numberText.contains(Text1))
                        {
                            float gradientPosition = static_cast<float>(std::distance(input.begin(), std::find(input.begin(), input.end(), j))) / (input.size() - 1);
                            juce::Colour color = gradient.getColourAtPosition(gradientPosition);
                            g.setColour(color);
                        }
                    ...
                    g.fillEllipse(weightX - circleSize / 2, weightY - circleSize / 2, circleSize, circleSize);
                    g.setColour(juce::Colours::black);
                    g.drawText(weightText, weightX - circleSize / 2, weightY - circleSize / 2, circleSize, circleSize, juce::Justification::centred);
                    g.setColour(juce::Colours::black);
                }
            }
        }
    ...
    void buttonClicked(juce::Button* button) override
    {
        ...
        {
            input = solveTSP(graph.getAdjacencyMatrix());
            juce::String numberText1;
            if (input.size() > 1)
            {
                for (auto number : input) numberText1 += juce::String(number + 1) + " ";
                for (auto number : input) numberText += juce::String(number);
            }
            else numberText = "No found.";
            numberField.setBounds(10, 290, 100, 30);
            numberField.setColour(juce::TextEditor::textColourId, juce::Colours::white);
            numberField.setText(numberText1);
